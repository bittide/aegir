...
// Same topology as sum_array, but with optional outputs of struct type.
...
fn sum_requests() -> anyhow::Result<()> {
    ...
    */
    ...
    // create the nodes
    let accum = action!("accum" in app_spec () (x: [Option<Request>; N_INPUTS], en: Option<u64>)
                -> (sum: Option<u64>) {} {
                    sum = if let Some(_) = en {
                        Some(x.iter()
                             .map(|r|
                                  if let Some(req) = *r { req.data.iter().sum() } else { 0 } )
                             .sum())
                    } else {
                        None
                    }
    ...
    let input_nodes = (0..N_INPUTS)
        .map(|i| {
            ...
            (counter: u64) () -> (output: Option<Request>)
            { u64::pack(&0).into_boxed_bitslice() }
            {
                counter += 1;
                output = Some(Request::new(counter));
            })
        })
        .collect::<Vec<_>>();
    ...
    (0..MAX_ITER).for_each(|i| {
        ...
        if i > DELAY {
            if i % 2 == 0 {
                assert_eq!(
                    u64::unpack(&app_spec.get_node(sink).borrow().persistent_state().unwrap()),
                    (N_INPUTS as u64 * N_ELEMENTS as u64) * (i - DELAY + 1)
                );
            } else {
                assert_eq!(
                    u64::unpack(&app_spec.get_node(sink).borrow().persistent_state().unwrap()),
                    (N_INPUTS as u64 * N_ELEMENTS as u64) * (i - DELAY)
                );
            }
        }
    });
...
#[test]
fn test_opt_in_array() {
    sum_requests().expect("Failed simulation");
}
